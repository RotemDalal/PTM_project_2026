...
public class Topic {
    ...
    public void subscribe(Agent a) { if (!subs.contains(a)) subs.add(a); }
    public void unsubscribe(Agent a) { subs.remove(a); }
    public void publish(Message m) { for (Agent a : subs) a.callback(name, m); }
    public void addPublisher(Agent a) { if (!pubs.contains(a)) pubs.add(a); }
